...
11  E or W
(12) NMEA 2.3 and later: FAA mode indicator, might or might not be in the message depending on gps 
...
def setup_port():
    ...
    try:
        ...
        serport = serial.Serial(port=const_gps_serialport,
                                baudrate=const_gps_baudrate,
                                timeout=0.5)
        ...
        log.error(
            f'(SETUP_PORT) Error while setting up GNSS port (ARE YOU ROOT?): {e}'
        )
        return f'ERROR (SETUP_PORT): While setting up GNSS port (ARE YOU ROOT?): {e}'
...
def get_nmea(timeout=45):
    ...
    serport.close(
    )  # reset port, got random "device reports readiness to read but returned no data (device disconnected or multiple access on port?)" errors
    serport.open()
    ...
    while timeout:
        while serport.inWaiting(
            ...
            if uart_buffer_str[
                ...
                if full_string[
                        0:
                        7] == '$GPGGA,':  # Check if it is a GGA message
                    if __check_checksum(full_string):
                        if __check_gps_quality(
                                full_string
                        ) > 0:  # check the message for checksum errors and invalid fixes
                ...
                if full_string[
                        0:
                        7] == '$GPRMC,' and valid_gga != '':  # we only want to check the RMC message after a valid gps fix
                    if __check_checksum(
                            full_string
                    ):  # Check if it is a (valid) RMC message
...
if __name__ == '__main__':
    try:
        setup_port()
        log.info(
            'Succesfully initialized port {} at {} baud'.
            format(const_gps_serialport,
                   const_gps_baudrate))
        log.info('Waiting for GNSS data...\n')
    except Exception as e:
        log.error(
            'Exception while opening port (are you root?): {}'
            .format(e))
        exit()
...
while __name__ == '__main__':
    try:
        ...
        log.warning(
            'Intercepted a CTRL + C command, now exiting...'
        )
        exit()
    except Exception as e:
        log.error('{}\n'.format(e))
